...
int sys__create_thread ( void *func, void *param, int prio, thread_t *thread )
{
	...
	ASSERT_ERRNO_AND_SYS_EXIT ( func, E_PARAM_NULL );
...
int sys__wait_for_thread ( thread_t *thread, int wait )
{
	...
	ASSERT_ERRNO_AND_SYS_EXIT ( thread && thread->thread, E_INVALID_HANDLE );
...
int sys__thread_self ( thread_t *thread )
{
	...
	ASSERT_ERRNO_AND_SYS_EXIT ( thread, E_INVALID_HANDLE );
